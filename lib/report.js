...
function classifyFlag(flag, ctx = {}) {
  ...
  if (/^http \d|^\[?http \d/.test(f)) return 'high';
  if (/missing <title>|no <h1>|missing <meta name="viewport"/.test(f)) return 'high';
  if (/no <form>|no conversion path|no booking\/appointment cta|no styled cta buttons/.test(f)) return 'high';
  // MEDIUM
  if (/multiple <h1>|title too short|title too long|meta description too|missing meta description/.test(f)) return 'medium';
  if (/no "privacy policy"|no "terms" link|no "cookie policy"/.test(f)) return 'medium';
  if (/footer does not contain "allied health media"/.test(f)) return 'medium';
  if (/no gtm container|missing <link rel="canonical|no json-ld structured|missing <html lang/.test(f)) return 'medium';
  if (/no clickable phone|no social proof|no medical or local-business schema|many competing ctas/.test(f)) return 'medium';
  if (/no gmc number visible/.test(f)) return 'medium';
  // LOW
  return 'low';
}
