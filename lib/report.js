...
// Wrap a Buffer in an array so Readable.from emits the whole buffer as ONE
// chunk instead of iterating it byte-by-byte (which yields numbers and breaks
// the googleapis upload pipe with ERR_INVALID_ARG_TYPE: "chunk … Received
// type number").
...
async function saveReportToDrive({ md, fileName, parentFolderId, makePublicLink = true }) {
  ...
  const media = { mimeType: 'text/markdown', body: bufferStream(buffer) };
...
async function savePdfToDrive({ pdfBuffer, fileName, parentFolderId, makePublicLink = true }) {
  ...
  const media = { mimeType: 'application/pdf', body: bufferStream(pdfBuffer) };
  const res = await drive.files.create({ requestBody, media, fields: 'id, webViewLink, name' });
  if (makePublicLink) {
    try { await makePublic(res.data.id); }
    catch (err) { console.warn(`makePublic failed for ${res.data.id}: ${err.message}`); }
